...
import {
  ...
  PaginationEllipsis,
  getPaginationRange,
} from "./ui/pagination"; // Fix import path for pagination components
...
export function DataTable<T extends { id: number | string }>({
      ...
      <div className={classNames(
        ...
        {layoutSettings.showRecordInfo && (
          <div className={classNames(
            ...
            responsiveSettings.compactMode && isMobileView && "text-xs text-center sm:text-left"
          )}>
  ...
  return (
    <div className={classNames("space-y-4", className)}>
      <div className={classNames(
        layoutSettings.toolbarLayout === 'stacked'
          ? "flex flex-col gap-4"
          : layoutSettings.toolbarLayout === 'compact'
          ? "flex flex-row items-center gap-2 justify-between"
          : "flex flex-col sm:flex-row gap-4 sm:items-end sm:justify-between"
      )}>
        {/* Search and Filters Row */}
        <div className={classNames(
          layoutSettings.searchPosition === 'full'
            ? "flex flex-col gap-2 w-full"
            : layoutSettings.filtersLayout === 'wrapped'
            ? "flex flex-col gap-2 flex-1"
            : "flex flex-col sm:flex-row gap-2 flex-1"
        )}>
          <Input
                  ...
                  >
                    <SelectTrigger
                      className={classNames(
                        ...
                        responsiveSettings.compactMode && isMobileView && "text-xs h-8",
        ...
        <div className={classNames(
          ...
          layoutSettings.actionsPosition === 'left'
            ? "justify-start"
            : layoutSettings.actionsPosition === 'bottom'
            ? "justify-center w-full"
            : "justify-end sm:ml-auto",
          layoutSettings.toolbarLayout === 'compact' && "flex-shrink-0"
        )}>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      ...
                      className={classNames(
                        "w-full sm:w-auto",
                        responsiveSettings.compactMode && isMobileView && "text-xs px-2 py-1"
                      )}
                    >
          ...
          {enableColumnVisibility && (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  <Settings2 className={classNames(
                    "mr-2",
                    responsiveSettings.compactMode && isMobileView ? "h-3 w-3" : "h-4 w-4"
                  )} />
          ...
          <div className="flex items-center gap-1 whitespace-nowrap">
            <span className={classNames(
              "text-sm",
              responsiveSettings.compactMode && isMobileView && "text-xs hidden sm:inline"
            )}>
            ...
            >
              <SelectTrigger
                className={classNames(
                  "min-w-0 w-18",
                  responsiveSettings.compactMode && isMobileView && "text-xs h-7 w-14",
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={activeFilterGroup.rules.length === 0}
              >
                Clear All
              </Button>
              <Button
                  ...
                  {index > 0 && (
                    <Select
                      ...
                      onValueChange={(value: 'AND' | 'OR') =>
                        updateFilterRule(index, { condition: value })
                      }
                    >
                    ...
                    {(() => {
                      ...
                      if (field.type === 'select' || field.type === 'multiselect') {
                        ...
                        if (shouldUseMultiSelect) {
                          ...
                          const select2Options = field.options?.map(opt => ({
                            id: String(opt.value),
                            label: opt.label
                          })) || [];
                          ...
                          if (rule.value) {
                            if (Array.isArray(rule.value)) {
                              currentValue = rule.value.map(v => ({ id: String(v), label: field.options?.find(opt => opt.value === v)?.label || String(v) }));
                            } else if (typeof rule.value === 'string' && rule.value.includes(',')) {
                              // Handle comma-separated values
                              const values = rule.value.split(',');
                              currentValue = values.map(v => ({ id: v.trim(), label: field.options?.find(opt => opt.value === v.trim())?.label || v.trim() }));
                            } else {
                              currentValue = [{ id: String(rule.value), label: field.options?.find(opt => opt.value === rule.value)?.label || String(rule.value) }];
                            }
                          }
                      ...
                      if (field.type === 'date') {
                        return (
                          <DatePicker
                            ...
                            onChange={(date) => updateFilterRule(index, {
      ...
      <div className={classNames(
        "relative",
        responsiveSettings.enableResponsive && isMobileView && "overflow-x-auto"
      )}>
                ...
                >
                  <div className="flex items-center gap-1">
                    {enableColumnReordering && (
                      <GripVertical className="w-4 h-4 opacity-0 group-hover:opacity-50 cursor-grab active:cursor-grabbing" />
                    )}
                    {col.label}
            ...
            {loading ? (
              <TableRow>
                <TableCell colSpan={
                  ...
                  (actions ? 1 : 0) +
                  (selectionSettings.enableRowSelection ? 1 : 0)
                }>
                  ...
                  {responsiveColumns.map((col) => (
                    ...
                    >
                      {responsiveSettings.mobileStackedView && isMobileView && (
                        <span className="font-medium text-muted-foreground mr-2 md:hidden">
